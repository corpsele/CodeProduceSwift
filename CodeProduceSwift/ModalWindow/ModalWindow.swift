...
//        center()
        ...
        NotificationCenter.default.addObserver(forName: NSApplication.didResignActiveNotification, object: nil, queue: OperationQueue.main) { [weak self] _ in
            self?.level = .floating
        }
...
//            let vc = bWebSB.instantiateController(withIdentifier: "BWebController") as? BWebController
            ...
            let cvc = container.resolve(BWebController.self)
            cvc?.view.frame = rect
            let presentAnimator = CustomVCPresentAnimator()
//            self.mainVC?.present(cvc ?? bWebController, animator: presentAnimator)
            self.mainVC?.presentAsModalWindow(cvc ?? bWebController)
        }
    }
...
//        }
        setFrame(NSRect(x: ePoint.x, y: ePoint.y, width: frame.width, height: frame.height), display: true, animate: true)
        windowX = ePoint.x
        windowY = ePoint.y
    }
...
//        menuMouse?.popUp(positioning: nil, at: ePoint, in: NSApp.windows.first?.contentView)//NSApp.mainWindow?.contentView
        NSMenu.popUpContextMenu(menuMouse!, with: event, for: contentView!)
    }
...
class ClockView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        ss.draw(in: NSOffsetRect(frame, frame.width / 2 - textRect.width / 2, -(frame.height / 2 - textRect.height / 2)), withAttributes: [NSAttributedString.Key.foregroundColor: NSColor.white, NSAttributedString.Key.font: NSFont.systemFont(ofSize: 32.0)])
    }
